...
module Images
  class Preprocessor
    ...
    def self.augmentation(dataset_id, params)
      imgs = Dataset.find(dataset_id).images
      ...
      Parallel.each(imgs, in_threads: 8) do |img|
        ...
        if rotate
          output_path_rotated = "#{OUTPUT_DIRECTORY}/#{img.name}_rotated_#{rotation_angle}.jpg"
          process_image_with_rotation(input_path, output_path_rotated, width, height, rotation_angle)
        end

        if zoom
          output_path_zoomed = "#{OUTPUT_DIRECTORY}/#{img.name}_zoomed.jpg"
          process_image_with_zoom(input_path, output_path_zoomed, width, height, 1.5)
        end

        if mirror
          output_path_mirrored = "#{OUTPUT_DIRECTORY}/#{img.name}_mirrored.jpg"
